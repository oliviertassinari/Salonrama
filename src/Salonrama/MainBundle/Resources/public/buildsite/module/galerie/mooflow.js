...
getElements: function(el)
{
	...
	$$(els).each(function(el)
	{
		...
		if(el.get('tag') == 'a'){ hash.combine(el.getProperties('href')); }
...
{
	this.aniFx2 = new Fx2.Value({
		'transition': Fx2.Transitions.Expo.easeOut,
		'link': 'cancel',
		'duration': 750,
...
update: function(e)
{
	...
	if(this.options.useSlider)
	{
		this.nav.setStyle('width', this.nav.getSize().x);
...
{
	this.index = index;
	this.aniFx2.start(this.aniFx2.get(), index*-this.foc);
},

getSizeOpt: function(ActW, ActH, OptW, OptH)
{
	if(ActW > OptW || ActH > OptH)
	{
...
process: function(x)
{
	...
	var oH = this.MooFlow.getSize().y*0.65;
	...
	this.master.images.each(function(el)
	{
		...
		if(-foc*6< x && x < foc*6)
		{
			with(Math)
			{
				...
				var ImageSizeOpt = self.getSizeOpt(W, H, oH, oH);
				W = ImageSizeOpt.w;
				H = ImageSizeOpt.h;
...
compute: function(from, to, delta){
	this.value = Fx2.compute(from, to, delta);
...
{
	GModule.ScriptCharger('Galerie', 'creator/module/galerie/mooflow.js');
}
